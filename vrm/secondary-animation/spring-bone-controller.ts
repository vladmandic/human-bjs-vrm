...
export class SpringBoneController {
  ...
  public constructor(
    ...
    const colliderGroups = this.constructColliderGroups(bone);
    this.springs = this.constructSprings(bone, colliderGroups);
  }
  ...
  public async update(deltaTime: number): Promise<void> {
    ...
    const promises = this.springs.map<Promise<void>>((spring) => spring.update(deltaTime));
    return Promise.all(promises).then(() => { /* Do nothing */ });
  }

  private constructColliderGroups(localBone: getBone) {
    ...
    this.ext.colliderGroups.forEach((colliderGroup) => {
      const bone = localBone(colliderGroup.node) as TransformNode;
  ...
  private constructSprings(localBone: getBone, colliderGroups: ColliderGroup[]) {
    if (!this.ext.boneGroups || !this.ext.boneGroups.length) {
      return [];
    }
    const springs: VRMSpringBone[] = [];
    this.ext.boneGroups.forEach((spring) => {
      const rootBones = (spring.bones || []).map((bone) => localBone(bone) as TransformNode);
      const springColliders = (spring.colliderGroups || []).map<ColliderGroup>((g) => colliderGroups[g]);
